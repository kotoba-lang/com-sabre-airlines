(ns sabre-airlines.main-test
  (:require [clojure.test :refer [deftest is testing run-tests]]
            [clojure.string :as str]
            [sabre-airlines.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "sabre_airlines-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'sabre-airlines.main-test)]
