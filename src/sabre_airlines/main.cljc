(ns sabre-airlines.main
  "Kotodama WASM entrypoint for the Sabre_airlines clean-room actor (L4) — Clojure port.
  ...
  facts (`sabre_airlines.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sabre_airlines")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sabre_airlines.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sabre_airlines-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sabre_airlines-compat/ready)
